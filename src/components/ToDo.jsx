...
const ToDo = () => {
  ...
  const [filterTasks, setFilterTasks] = useState(userTasks);

  //to store todolist data in localstorage
  useEffect(() => {
  ...
  const taskGetFun = (data) => {
    ...
    //to go todolist component
    setHidden(false);
  };
  // edit to do handler
  const editToDoHandler = (id) => {
    const editToDo = filterTasks.map((todo) =>
  ...
  const updateToDoHandler = (tsk, id) => {
    const updateToDo = filterTasks.map((todo) =>
      todo.id === id ? { ...todo, task: tsk, isEditing: !todo.isEditing } : todo
    );
    setUserTasks(updateToDo);
  ...
  return (
    <div className="bg-slate-100 w-[400px] min-h-[200px] rounded-md relative">
      <div className="pt-4 pb-16 px-6 sm:px-6">
        <Header />
        {hidden ? (
